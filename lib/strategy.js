...
function Strategy(options, verify) {
  ...
  if (!verify) { throw new TypeError('Asgardeo requires a verify function'); }
  if (!options.issuer) { throw new TypeError('Asgardeo requires an issuer option'); }
  if (!options.authorizationURL) { throw new TypeError('Asgardeo requires an authorizationURL option'); }
  if (!options.tokenURL) { throw new TypeError('Asgardeo requires a tokenURL option'); }
  if (!options.clientID) { throw new TypeError('Asgardeo requires a clientID option'); }
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    function stored(err, state) {
      if (err) { return self.error(err); }
      if (!state) { return self.error(new Error('Asgardeo state store did not yield state for authentication request')); }
